...
float4 MainPS(float4 color : COLOR0, float2 texCoord : TEXCOORD0) : COLOR0
{
    ...
    float distortion = 0.075; // Subtle curvature (matching CRTSubtle)
    ...
    float aberrationStrength = length(centerOffset) * 0.015; // Stronger at edges
    ...
    // Scanlines using sin wave (avoids if statements)
    float scanline = sin(uv.y * 3.14159 * 224.0);
    scanline = scanline * 0.5 + 0.5; // Convert from -1,1 to 0,1
    texColor.rgb *= 0.75 + (scanline * 0.25); // Subtle scanline effect

    // Vignette - hardcoded strength
    float2 vigUV = uv - 0.5;
    float dist = length(vigUV) * 2.0;
    float vignette = 1.0 - (dist * 0.4);
    texColor.rgb *= vignette;

    // Brightness boost - CRTs were brighter and more vibrant
    texColor.rgb *= 1.15;
