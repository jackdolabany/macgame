...
{
    public class BigAlienShip : EnemyShipBase
    {
        private const float FireInterval = 2f;
        private const float ShotSpeed = 150f;

        public BigAlienShip(ContentManager content, int cellX, int cellY, Player player, Camera camera)
            : base(content, cellX, cellY, player, camera)
        {
            ...
            var idle = new AnimationStrip(textures, Helpers.GetBigTileRect(12, 8), 4, "idle");
            ...
            enemyShipBehavior.SetupShootAtPlayer(FireInterval, ShotSpeed, ShotSize.Medium, 3.5f);
